...
function load_overrides_map($pdo, $rtId, $planId, array $dates): array {
    ...
    $sql = "SELECT stay_date, price_minor FROM rate_plan_prices WHERE room_type_id = ? AND rate_plan_id = ? AND stay_date IN ($in)";
    $params = array_merge([(int)$rtId, (int)$planId], $dates);
    $st = $pdo->prepare($sql); $st->execute($params);
    $map = [];
    while($row=$st->fetch(PDO::FETCH_ASSOC)){
        $map[$row['stay_date']] = ['price_minor'=>(int)$row['price_minor']];
    }
    return $map;
}
...
foreach ($roomTypes as $rt) {
    ...
    foreach ($ratePlans as $rp) {
        ...
        foreach ($stayDates as $d){
            if (isset($overMap[$d])) {
                $datesOut[] = [ 'stay_date'=>$d, 'price_minor'=>(int)$overMap[$d]['price_minor'], 'source'=>'override' ];
            } else {
                $datesOut[] = [ 'stay_date'=>$d, 'price_minor'=>(int)$rp['base_price_minor'], 'source'=>'base' ];
            }
        }

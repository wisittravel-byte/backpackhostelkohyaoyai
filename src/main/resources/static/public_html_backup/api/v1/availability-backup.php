...
function load_amenities($pdo, $rtId){
    ...
    try{ 
        ...
        // Add web-accessible icon URL. Data may point to:
        // - /assets/image/amenities/... (NEW: must be served under /public/)
        // - assets/uploads/... (CMS uploads)
        // - static/public/... or public/... (legacy checked-in static)
        foreach($rows as &$row){
            ...
            if (preg_match('#^https?://#i', $iconPath)) {
                ...
                // NEW: Static site icons under /assets/image/amenities/...
                // Must be served from /public/assets/... on both local and production
                $row['icon'] = '/public/' . $raw;
            } elseif (strpos($raw, 'static/public/') === 0) {
                // Legacy: Map filesystem path to web root (strip 'static/')
                $row['icon'] = '/' . substr($raw, strlen('static/'));

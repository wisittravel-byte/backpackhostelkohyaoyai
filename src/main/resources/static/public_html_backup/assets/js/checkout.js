(function(){
  ...
  const taxConfig = await fetchTaxConfig();
      ...
      try{
        if(!bookingData.hold_id && bookingData.room_type_id && checkIn && checkOut){
          ...
          let res;
          try{ res = await fetch(primaryUrl, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload), cache:'no-cache' }); }catch(_){ res = null; }
          if(!(res && res.ok)){
            if(!(res && res.status===409)){
              try{ res = await fetch(fallbackUrl, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload), cache:'no-cache' }); }catch(_e){ res = null; }
            }
          }
          if(res && res.ok){
  ...
  async function releaseHoldIfAny(reason){
    ...
    const apiBase = (window.location.port === '8080') ? 'https://backpackkohyao.com' : '';
    try{
      const res = await fetch(`${apiBase}/api/v1/inventory-hold-release.php`, {
        method:'POST', 
        headers:{'Content-Type':'application/json'}, 
        body: JSON.stringify({ hold_id: holdId, updated_by: reason||'website' }), 
        cache:'no-cache', 
        keepalive:true
      });
      ...
      console.log(`✅ Hold ${holdId} released by ${reason}, API response:`, res.ok);
      return res.ok;
    }catch(err){
  ...
  function setupChangeDatesButton(){
    const changeDatesBtn = document.getElementById('changeDatesBtn');
    if(changeDatesBtn){
      changeDatesBtn.addEventListener('click', async ()=>{
        console.log('🔄 Change dates button clicked');
        try{ 
          const released = await releaseHoldIfAny('user_change_dates');
  ...
  function startHoldCountdown(bookingData){
    ...
    if(!exp || !holdId) return;
    let end = Date.parse(exp.endsWith('Z') ? exp : (exp + 'Z'));
  ...
  async function expireHoldNow(holdId){
    ...
    try{
      ...
      try{ res = await fetch(primaryUrl, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload), cache:'no-cache' }); }catch(_){ res = null; }
      if(!(res && res.ok)){
        try{ await fetch(fallbackUrl, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload), cache:'no-cache' }); }catch(_e){}
      }
      // Clean local storage
      try{
        const raw = localStorage.getItem('booking_data');
      ...
      try{
        ...
        try{
          res = await fetch(`${apiBase}/api/v1/inventory-hold-release.php`, {
            method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload), cache:'no-cache', keepalive:true
          });
        }catch(_e){ res = null; }
        if(!(res && res.ok)){
          try{
            res = await fetch(`${apiBase}/php-api/v1/inventory-hold-release.php`, {
              method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload), cache:'no-cache', keepalive:true
            });
          }catch(_e2){ res = null; }
        }
